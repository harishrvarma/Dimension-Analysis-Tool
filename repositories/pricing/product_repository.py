...
class ProductRepository(BaseRepository):
    ...
    def update_products_aggregated(self, product_updates: list):
        """Update products with aggregated results using chunked bulk CASE UPDATE"""
        from sqlalchemy import text
        ...
        for chunk_start in range(0, len(product_updates), CHUNK_SIZE):
            ...
            for i, update in enumerate(chunk):
                sid = update.get('system_product_id')
                if not sid:
                    continue
                ids.append(sid)
                params[f'sid{i}'] = sid
                ...
                if 'outlier_mode' in update:
                    params[f'mode{i}'] = update['outlier_mode']
                    mode_cases.append(f'WHEN :sid{i} THEN :mode{i}')

            if not ids:
                continue

            id_placeholders = ','.join([f':sid{i}' for i in range(len(ids))])
            set_parts = ['analyzed_date = NOW()']
